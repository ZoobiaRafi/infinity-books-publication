...
{{--
    This file overrides vendor/tcg/voyager/resources/views/index.blade.php
    (Laravel's standard package view-publish mechanism: resources/views/vendor/voyager/*
    takes priority over the package's own views). Only the "Mail Overview" block
    below was added - everything else is an unmodified copy of Voyager's stock
    dashboard, kept in sync manually since there's no extension point in the
    original file to hook into instead.
--}}
    ...
    <div class="page-content">
        ...
        @isset($mailAccounts)
            @if($mailAccounts->isNotEmpty())
                <div class="mail-app" style="margin-bottom: 20px;">
                    @include('admin.mail.partials.styles')
                    <h4 style="margin: 0 0 12px; font-weight: 700;">
                        <i class="voyager-mail"></i> {{ __('Mail Overview') }}
                    </h4>
                    <div class="mail-overview-grid">
                        @foreach($mailAccounts as $acct)
                            @php($hue = crc32($acct->label) % 360)
                            <div class="mail-overview-card">
                                <div class="mail-overview-head">
                                    <span class="mail-avatar" style="background: hsl({{ $hue }}, 55%, 48%);">
                                        {{ strtoupper(substr($acct->label, 0, 1)) }}
                                    </span>
                                    <div>
                                        <div class="mail-overview-title">{{ $acct->label }}</div>
                                        <div class="mail-overview-sub">{{ $acct->email_address }}</div>
                                    </div>
                                </div>
                                <div class="mail-overview-stats">
                                    <div class="mail-stat-unread">
                                        <div class="mail-stat-num">{{ $acct->unread_count }}</div>
                                        <div class="mail-stat-label">{{ __('Unread') }}</div>
                                    </div>
                                    <div>
                                        <div class="mail-stat-num">{{ $acct->opened_count }}</div>
                                        <div class="mail-stat-label">{{ __('Opened') }}</div>
                                    </div>
                                    <div>
                                        <div class="mail-stat-num">{{ $acct->inbox_total }}</div>
                                        <div class="mail-stat-label">{{ __('Inbox') }}</div>
                                    </div>
                                </div>
                                <a href="{{ route('admin.mail.inbox', $acct) }}" class="btn btn-primary">{{ __('Open Mailbox') }}</a>
                            </div>
                        @endforeach
                    </div>
                </div>
